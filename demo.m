...
lambda=0.01;
    ...
    [Z,tlrr_E,Z_rank,err_va ] = ADMM(X,L,max_iter,lambda);
a=toc;
...
AUC = sum((PF_40(1:end-1)-PF_40(2:end)).*(PD_40(2:end)+PD_40(1:end-1))/2)

f_show=reshape(r_new,[H,W]);
f_show=(f_show-min(f_show(:)))/(max(f_show(:))-min(f_show(:)));
figure('name','WMS-LRTR'), imshow(f_show);
